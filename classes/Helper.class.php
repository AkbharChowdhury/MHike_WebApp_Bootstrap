...
final class Helper {
    ...
    public static function calcElevationPerMile($distance, $elevationGain) {
        return doubleval($elevationGain) / (doubleval($distance) / 2);

    }

    public static function getDifficultyLevel($distance, $elevationGain) {
        $total = self::calcElevationPerMile($distance, $elevationGain);
        $result = $total;

        if (in_array($result, range(200,400))) {
            return 0; 
            // Easy
        }
        if (in_array($result, range(400,700))) {
            return 1; // Moderate
        }
        if (in_array($result, range(700,1000))) {
            return 2; // Hard
        }
